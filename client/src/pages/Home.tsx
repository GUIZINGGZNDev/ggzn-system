  ...
  return <main className="min-h-screen bg-[#f6f6f2] text-black selection:bg-black selection:text-white">
    <header className="sticky top-0 z-30 border-b-2 border-black bg-[#f6f6f2]/95 px-5 py-5 backdrop-blur-md md:px-10">
      <div className="mx-auto flex max-w-[1400px] items-start justify-between gap-6">
        <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-[1px] bg-black text-xl font-black text-lime-300 shadow-[4px_4px_0_#b8d92f]">G</div><span className="text-sm font-black tracking-[0.26em]">GGZN SYSTEM</span></div>
        <nav className="hidden gap-8 text-xs font-black uppercase tracking-[0.14em] md:flex"><a href="#comandos" className="underline decoration-2 underline-offset-4">Comandos</a><a href="#converse">Converse com o bot</a><a href="#arquitetura">Arquitetura</a></nav>
        <a className="text-xs font-black uppercase underline decoration-2 underline-offset-4" href="#converse">Converse com o bot <ArrowDownRight className="inline h-4 w-4" /></a>
      </div>
    </header>

    <section className="mx-auto grid max-w-[1400px] gap-10 px-5 pb-24 pt-16 md:grid-cols-[1.15fr_.85fr] md:px-10 md:pt-24">
      <div><div className="mb-8 flex items-center gap-3 text-xs font-black uppercase tracking-[0.18em]"><span className="h-3 w-3 animate-pulse bg-lime-400 shadow-[0_0_0_5px_rgba(217,255,87,.25)]" /> ONLINE / NODE.JS / BAILEYS</div><h1 className="max-w-5xl text-[clamp(4.5rem,13vw,12.5rem)] font-black leading-[.78] tracking-[-0.095em]">BOT<br /><span className="ml-[9vw]">BRUTO.</span></h1><p className="mt-12 max-w-xl border-l-8 border-black pl-5 text-xl font-bold leading-tight md:text-2xl">Automação de grupo sem interface enfeitada. Comandos rápidos, cargos claros e controle persistente por grupo.</p><div className="mt-10 flex flex-wrap gap-3"><a href="#comandos"><Button className="h-14 rounded-none border-4 border-black bg-black px-7 text-sm font-black uppercase tracking-widest text-white hover:bg-lime-300 hover:text-black">Ver comandos <ArrowDownRight className="ml-3 h-5 w-5" /></Button></a><a href="#converse"><Button variant="outline" className="h-14 rounded-none border-4 border-black bg-white px-7 text-sm font-black uppercase tracking-widest hover:bg-black hover:text-white">Converse com o bot</Button></a></div></div>
      <div className="group relative flex min-h-[380px] items-end overflow-hidden border-4 border-black bg-[#111] p-6 text-white shadow-[10px_10px_0_#d9ff57] md:min-h-[510px]" style={{ backgroundImage: 'linear-gradient(180deg, rgba(5,5,5,.08), rgba(5,5,5,.86)), url(/manus-storage/ggzn-premium-hero_eecc6acd.webp)', backgroundSize: 'cover', backgroundPosition: 'center' }}><div className="absolute right-0 top-0 h-24 w-24 border-b-4 border-l-4 border-white/50 bg-orange-300" /><div className="absolute left-6 top-6 text-[10px] font-black uppercase tracking-[.3em] text-lime-300">/ 01 — manifesto</div><div className="relative"><div className="mb-4 text-[8rem] font-black leading-[.72] tracking-[-.12em] text-white drop-shadow-[5px_5px_0_#d9ff57]">GG<br />ZN</div><div className="flex items-center gap-2 border-t-2 border-white/50 pt-4 text-xs font-black uppercase"><Zap className="h-4 w-4 text-lime-300" /> sem conversa mole</div></div></div>
    </section>
